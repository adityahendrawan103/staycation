...
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Layanan</span>
            <span class="text-muted mt-1 fw-bold fs-7">Form layanan reservasi</span>
        </h3>
        <div class="card-toolbar">
            <button id="btnTambah" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEntryLayanan">
    ...
    <div class="card-body p-9">
        <div class="fv-row mb-8">
            <label class="form-label">Cari Data Layanan:</label>
            <div class="input-group">
                <span class="input-group-text">Pencarian</span>
                <input id="inputSearch" name="search" type="search" class="form-control" placeholder="Cari Data Layanan"
        ...
        <div class="fv-row">
            <div class="table-responsive">
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    ...
                    <tbody class="border">
                        @forelse($data_layanan->data as $data)
                        <tr class="fs-6 fw-bold text-gray-700">
                            <td class="ps-3 pe-3">{{ trim($data->kode_layanan) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->nama_layanan) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->satuan) }}</td>
                            <td class="ps-3 pe-3 text-end">{{ number_format($data->harga) }}</td>
                            <td class="ps-3 pe-3">
                                <button id="btnEdit" class="btn btn-icon btn-light-primary btn-sm" data-toggle="modal" data-kode={{ trim($data->kode_layanan) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <button id="btnHapus" class="btn btn-icon btn-light-danger btn-sm" data-kode={{ trim($data->kode_layanan) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <tr class="fw-bolder text-muted fs-6">
                            <td class="text-center p-20" colspan="4">- TIDAK ADA DATA YANG DITAMPILKAN -</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                    <div class="dataTables_paginate paging_simple_numbers">
                        <ul class="pagination">
                            @foreach ($data_layanan->links as $link)
...
<div id="modalEntryLayanan" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div id="modalContentLayanan" class="modal-content">
            <form id="formEntryLayanan" name="formEntryLayanan" autofill="off" autocomplete="off" method="post" action="{{ route('master.layanan.simpan-layanan') }}">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="fv-row">
                        ...
                        <input id="inputKodeLayanan" name="kode_layanan" type="text" class="form-control" placeholder="Input Kode Layanan" maxlength="10" oninput="this.value = this.value.toUpperCase()" required>
                        <span id="messageKodeLayanan" class="invalid-feedback"></span>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Nama Layanan:</label>
                        <input id="inputNamaLayanan" name="nama_layanan" type="text" class="form-control" placeholder="Input Nama Layanan" maxlength="50" required>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Satuan:</label>
                        <select id="selectSatuan" name="satuan" class="form-select">
                    ...
                    <div class="fv-row mt-8">
                        ...
                        <input id="inputHarga" name="harga" type="text" class="form-control" placeholder="Input Harga" maxlength="13" required
...
<script type="text/javascript">
    ...
    const url = {
        'data_layanan': "{{ route('master.layanan.daftar-layanan') }}",
        'form_layanan': "{{ route('master.layanan.form-layanan') }}",
        'hapus_layanan': "{{ route('master.layanan.hapus-layanan') }}",
        'cek_kode_layanan': "{{ route('master.layanan.cek-kode-layanan') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/master/layanan/layanan.js') }}?v={{ time() }}"></script>
@endpush
@endsection
