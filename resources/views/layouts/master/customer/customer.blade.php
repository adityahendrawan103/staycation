...
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Customer</span>
            <span class="text-muted mt-1 fw-bold fs-7">Form customer</span>
        </h3>
        <div class="card-toolbar">
            <a href="{{ route('master.customer.form-add-customer') }}" id="btnTambah" class="btn btn-primary">
    ...
    <div class="card-body p-9">
        <div class="fv-row mb-8">
            <label class="form-label">Cari Data Customer:</label>
            <div class="input-group">
                <span class="input-group-text">Pencarian</span>
                <input id="inputSearch" name="search" type="search" class="form-control" placeholder="Cari Data Customer"
                ...
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    <thead class="border">
                        <tr class="fs-7 fw-bolder text-gray-500">
                            ...
                            <th class="w-50px text-center ps-3 pe-3">TglLahir</th>
                            <th class="min-w-150px ps-3 pe-3">Alamat</th>
                            <th class="min-w-150px ps-3 pe-3">Contact</th>
                            <th class="w-50px ps-3 pe-3 text-center">Registrasi</th>
                            <th class="min-w-100px ps-3 pe-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="border">
                        @forelse($data_customer->data as $data)
                        <tr class="fs-7 fw-bold text-gray-700">
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <span class="text-gray-800 fs-6 fw-bolder mb-1">{{ trim($data->no_identitas) }}</span>
                                <div class="text-gray-600 fs-7 fw-bolder mb-2">{{ trim($data->nama) }}</div>
                                @if(strtoupper(trim($data->jenis_identitas)) == 'KTP')
                                <span class="badge badge-light-primary fs-8 fw-boldest">KTP</span>
                                @elseif(strtoupper(trim($data->jenis_identitas)) == 'SIM')
                                <span class="badge badge-light-danger fs-8 fw-boldest">SIM</span>
                                @elseif(strtoupper(trim($data->jenis_identitas)) == 'PASPOR')
                                <span class="badge badge-light-info fs-8 fw-boldest">PASPOR</span>
                                @endif
                            </td>
                            <td class="ps-3 pe-3 text-center" style="text-align:left;vertical-align:top;">
                                <span class="text-gray-600 fs-7 fw-bolder">{{ date('d/m/Y', strtotime($data->tanggal_lahir)) }}</span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <span class="text-gray-600 fs-7 fw-bolder d-block">{{ trim($data->alamat) }}</span>
                                <span class="text-gray-600 fs-7 fw-bolder">{{ trim($data->kota) }}</span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <span class="text-gray-600 fs-7 fw-bolder d-block">{{ trim($data->email) }}</span>
                                <span class="text-gray-800 fs-7 fw-bolder">{{ trim($data->telepon) }}</span>
                            </td>
                            <td class="ps-3 pe-3 text-center" style="text-align:left;vertical-align:top;">
                                <span class="text-gray-600 fs-7 fw-bolder">{{ date('d/m/Y', strtotime($data->tanggal_registrasi)) }}</span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <a href="{{ route('master.customer.form-edit-customer', trim($data->no_identitas)) }}" id="btnEdit" class="btn btn-icon btn-light-primary btn-sm" data-toggle="modal" data-kode={{ trim($data->no_identitas) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <button id="btnHapus" class="btn btn-icon btn-light-danger btn-sm" data-kode={{ trim($data->no_identitas) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                    <div class="dataTables_paginate paging_simple_numbers">
                        <ul class="pagination">
                            @foreach ($data_customer->links as $link)
...
<script type="text/javascript">
    ...
    const url = {
        'data_customer': "{{ route('master.customer.daftar-customer') }}",
        'hapus_customer': "{{ route('master.customer.hapus-customer') }}",
    }
</script>
