...
@section('title','Room')
@section('subtitle','Tipe Ruangan')
@section('container')
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Tipe Ruangan</span>
            <span class="text-muted mt-1 fw-bold fs-7">Form tipe ruangan</span>
        </h3>
        <div class="card-toolbar">
            <button id="btnTambah" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEntryRoomTipe">
    ...
    <div class="card-body p-9">
        <div class="fv-row mb-8">
            <label class="form-label">Cari Data Tipe Ruangan:</label>
            <div class="input-group">
                <span class="input-group-text">Pencarian</span>
                <input id="inputSearch" name="search" type="search" class="form-control" placeholder="Cari Data Tipe Ruangan"
                ...
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    <thead class="border">
                        <tr class="fs-7 fw-bolder text-gray-500">
                            ...
                            <th class="min-w-50px ps-3 pe-3">Grade</th>
                            <th class="min-w-100px ps-3 pe-3 text-end">Harga</th>
                            <th class="min-w-100px ps-3 pe-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="border">
                        @forelse($data_tipe->data as $data)
                        <tr class="fs-6 fw-bold text-gray-700">
                            <td class="ps-3 pe-3">{{ trim($data->kode) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->nama_tipe) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->grade) }}</td>
                            <td class="ps-3 pe-3 text-end">{{ number_format($data->harga)}}</td>
                            <td class="ps-3 pe-3">
                                <button id="btnEdit" class="btn btn-icon btn-light-primary btn-sm me-1" data-toggle="modal" data-kode={{ trim($data->kode) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <tr class="fw-bolder text-muted fs-6">
                            <td class="text-center p-20" colspan="5">- TIDAK ADA DATA YANG DITAMPILKAN -</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                    <div class="dataTables_paginate paging_simple_numbers">
                        <ul class="pagination">
                            @foreach ($data_tipe->links as $link)
...
<div id="modalEntryRoomTipe" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div id="modalContentRoomTipe" class="modal-content">
            <form id="formEntryRoomTipe" name="formEntryRoomTipe" autofill="off" autocomplete="off" method="post" action="{{ route('master.room.tipe.simpan-tipe-room') }}">
                @csrf
                <div class="modal-header">
                    <h3 id="modalTitleRoomTipe" class="modal-title"></h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="fv-row">
                        ...
                        <input id="inputKodeTipe" name="kode_tipe" type="text" class="form-control" placeholder="Input Kode Tipe Ruangan" maxlength="10"
                            oninput="this.value = this.value.toUpperCase()" required>
                        <span id="messageKodeTipe" class="invalid-feedback"></span>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Nama Tipe Ruangan:</label>
                        <input id="inputNamaTipe" name="nama_tipe" type="text" class="form-control" placeholder="Input Nama Tipe Ruangan" maxlength="50" required>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Grade:</label>
                        <input id="inputGrade" name="grade" type="text" class="form-control" placeholder="Input Grade Tipe Ruangan" maxlength="1" oninput="this.value = this.value.toUpperCase()" required>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Tarif Harga:</label>
                        <input id="inputHarga" name="harga" type="text" class="form-control" placeholder="Input Tarif Harga" maxlength="13" required
...
<script type="text/javascript">
    ...
    const url = {
        'data_room_tipe': "{{ route('master.room.tipe.daftar-tipe-room') }}",
        'form_room_tipe': "{{ route('master.room.tipe.form-tipe-room') }}",
        'hapus_room_tipe': "{{ route('master.room.tipe.hapus-tipe-room') }}",
        'cek_kode_tipe': "{{ route('master.room.tipe.cek-kode-tipe-room') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/master/room/roomtipe.js') }}?v={{ time() }}"></script>
@endpush
@endsection
