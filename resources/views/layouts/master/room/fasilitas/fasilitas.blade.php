...
@section('title','Room')
@section('subtitle','Fasilitas')
@section('container')
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Fasilitas</span>
            <span class="text-muted mt-1 fw-bold fs-7">Form fasilitas ruangan</span>
        </h3>
        <div class="card-toolbar">
            <button id="btnTambah" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEntryFasilitas">
    ...
    <div class="card-body p-9">
        <div class="fv-row mb-8">
            <label class="form-label">Cari Data Fasilitas:</label>
            <div class="input-group">
                <span class="input-group-text">Pencarian</span>
                <input id="inputSearch" name="search" type="search" class="form-control" placeholder="Cari Data Fasilitas"
        ...
        <div class="fv-row">
            <div class="table-responsive">
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    ...
                    <tbody class="border">
                        @forelse($data_fasilitas->data as $data)
                        <tr class="fs-6 fw-bold text-gray-700">
                            <td class="ps-3 pe-3">{{ trim($data->kode_fasilitas) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->nama_fasilitas) }}</td>
                            <td class="ps-3 pe-3">
                                <button id="btnEdit" class="btn btn-icon btn-light-primary btn-sm" data-toggle="modal" data-kode={{ trim($data->kode_fasilitas) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <button id="btnHapus" class="btn btn-icon btn-light-danger btn-sm" data-kode={{ trim($data->kode_fasilitas) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                    <div class="dataTables_paginate paging_simple_numbers">
                        <ul class="pagination">
                            @foreach ($data_fasilitas->links as $link)
...
<div id="modalEntryFasilitas" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div id="modalContentFasilitas" class="modal-content">
            <form id="formEntryFasilitas" name="formEntryFasilitas" autofill="off" autocomplete="off" method="post" action="{{ route('master.room.fasilitas.simpan-fasilitas') }}">
                @csrf
                <div class="modal-header">
                    <h3 id="modalTitleFasilitas" class="modal-title"></h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="fv-row">
                        <label class="form-label required">Kode Fasilitas:</label>
                        <input id="inputKodeFasilitas" name="kode_fasilitas" type="text" class="form-control" placeholder="Input Kode Fasilitas" maxlength="10" oninput="this.value = this.value.toUpperCase()" required>
                        <span id="messageKodeFasilitas" class="invalid-feedback"></span>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Nama Fasilitas:</label>
                        <input id="inputNamaFasilitas" name="nama_fasilitas" type="text" class="form-control" placeholder="Input Nama Fasilitas" maxlength="50" required>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    const url = {
        'data_fasilitas': "{{ route('master.room.fasilitas.daftar-fasilitas') }}",
        'form_fasilitas': "{{ route('master.room.fasilitas.form-fasilitas') }}",
        'hapus_fasilitas': "{{ route('master.room.fasilitas.hapus-fasilitas') }}",
        'cek_kode_fasilitas': "{{ route('master.room.fasilitas.cek-kode-fasilitas') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/master/room/fasilitas.js') }}?v={{ time() }}"></script>
@endpush
@endsection
