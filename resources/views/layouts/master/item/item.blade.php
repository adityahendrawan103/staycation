...
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Item</span>
            <span class="text-muted mt-1 fw-bold fs-7">Form item ruangan</span>
        </h3>
        <div class="card-toolbar">
            <button id="btnTambah" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEntryItem">
    ...
    <div class="card-body p-9">
        <div class="fv-row mb-8">
            <label class="form-label">Cari Data Item:</label>
            <div class="input-group">
                <span class="input-group-text">Pencarian</span>
                <input id="inputSearch" name="search" type="search" class="form-control" placeholder="Cari Data Item"
                ...
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    <thead class="border">
                        <tr class="fs-7 fw-bolder text-gray-500">
                            ...
                            <th class="min-w-100px ps-3 pe-3 text-end">Denda</th>
                            <th class="min-w-100px ps-3 pe-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="border">
                        @forelse($data_item->data as $data)
                        <tr class="fs-6 fw-bold text-gray-700">
                            <td class="ps-3 pe-3">{{ trim($data->kode_item) }}</td>
                            <td class="ps-3 pe-3">{{ trim($data->nama_item) }}</td>
                            <td class="ps-3 pe-3 text-end">{{ number_format($data->harga_denda)}}</td>
                            <td class="ps-3 pe-3">
                                <button id="btnEdit" class="btn btn-icon btn-light-primary btn-sm" data-toggle="modal" data-kode={{ trim($data->kode_item) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <button id="btnHapus" class="btn btn-icon btn-light-danger btn-sm" data-kode={{ trim($data->kode_item) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                    <div class="dataTables_paginate paging_simple_numbers">
                        <ul class="pagination">
                            @foreach ($data_item->links as $link)
...
<div id="modalEntryItem" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div id="modalContentItem" class="modal-content">
            <form id="formEntryItem" name="formEntryItem" autofill="off" autocomplete="off" method="post" action="{{ route('master.item.simpan-item') }}">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="fv-row">
                        ...
                        <input id="inputKodeItem" name="kode_item" type="text" class="form-control" placeholder="Input Kode Item" maxlength="10" oninput="this.value = this.value.toUpperCase()" required>
                        <span id="messageKodeItem" class="invalid-feedback"></span>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Nama Item:</label>
                        <input id="inputNamaItem" name="nama_item" type="text" class="form-control" placeholder="Input Nama Item" maxlength="50" required>
                    </div>
                    <div class="fv-row mt-8">
                        <label class="form-label required">Harga Denda:</label>
                        <input id="inputHargaDenda" name="harga_denda" type="text" class="form-control" placeholder="Input Harga Denda" required
...
<script type="text/javascript">
    ...
    const url = {
        'data_item': "{{ route('master.item.daftar-item') }}",
        'form_item': "{{ route('master.item.form-item') }}",
        'hapus_item': "{{ route('master.item.hapus-item') }}",
        'cek_kode_item': "{{ route('master.item.cek-kode-item') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/master/item/item.js') }}?v={{ time() }}"></script>
@endpush
@endsection
