...
@section('subtitle','Check In')
@section('container')
@csrf
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-rooms-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Reservasi (Check In)</span>
        ...
        <div class="card-toolbar">
            <a href="{{ route('transaksi.reservasi.checkin.form-add-reservasi') }}" id="btnTambah" class="btn btn-primary">
                ...
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    <thead class="border">
                        <tr class="fs-7 fw-bolder text-gray-500">
                            ...
                            <th class="w-20px ps-3 pe-3 text-center">End</th>
                    ...
                    <tbody class="border">
                        ...
                        <tr class="fs-6 fw-bold text-gray-700">
                                ...
                                <div class="text-center">
                                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" value="1" style="pointer-events: none"
                                            @if($data->status_end == 1) checked @endif>
                                    </div>
                                </div>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                            ...
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <p class="text-gray-600 fs-7 fw-bolder">{{ trim($data->catatan) }}</p>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <a href="{{ route('transaksi.reservasi.checkin.form-edit-reservasi', trim($data->kode_reservasi)) }}" id="btnEdit" class="btn btn-icon btn-light-primary btn-sm mt-1" data-toggle="modal" data-kode={{ trim($data->kode_reservasi) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <button id="btnHapus" class="btn btn-icon btn-light-danger btn-sm mt-1" data-kode={{ trim($data->kode_reservasi) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <tr class="fw-bolder text-muted fs-6">
                            <td class="text-center p-20" colspan="10">- TIDAK ADA DATA YANG DITAMPILKAN -</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
<div id="modalFilter" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div id="modalContentFilter" class="modal-content">
            <form id="formEntryFilter" name="formEntryFilter" autofill="off" autocomplete="off" method="get" action="{{ route('transaksi.reservasi.checkin.daftar-reservasi') }}">
                ...
                <div class="modal-body">
                    <div class="fv-row">
                        ...
                        <select id="selectStatus" name="status" class="form-select">
                            <option value="ALL" @if(trim(strtoupper($data_filter->status)) == 'ALL') selected @endif>ALL</option>
                            <option value="BOOKED" @if(trim(strtoupper($data_filter->status)) == 'BOOKED') selected @endif>BOOKED</option>
                            <option value="NOTSHOW" @if(trim(strtoupper($data_filter->status)) == 'NOTSHOW') selected @endif>NOT SHOW</option>
                            <option value="NOTSHOWEND" @if(trim(strtoupper($data_filter->status)) == 'NOTSHOWEND') selected @endif>NOT SHOW END</option>
                            <option value="NOTSHOWENDNEEDPROCESS" @if(trim(strtoupper($data_filter->status)) == 'NOTSHOWENDNEEDPROCESS') selected @endif>NOT SHOW END (NEED PROCESS)</option>
                        </select>
                    </div>
...
<script type="text/javascript">
    ...
    const url = {
        'data_reservasi': "{{ route('transaksi.reservasi.checkin.daftar-reservasi') }}",
        'hapus_reservasi': "{{ route('transaksi.reservasi.checkin.hapus-reservasi') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/transaksi/reservasi/checkin/reservasi-daftar.js') }}?v={{ time() }}"></script>
@endpush
@endsection
