...
<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-7">
        <h3 class="card-title align-rooms-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Daftar Reservasi (History)</span>
            ...
            <div class="mt-4">
                ...
                @if(trim(strtoupper($data_filter->status)) != 'ALL')
                <span class="badge badge-light-primary fs-7 fw-boldest ms-2">
                    @if(trim(strtoupper($data_filter->status)) == 'BOOKED') BOOKED
                    @elseif(trim(strtoupper($data_filter->status)) == 'IN-HOUSE') IN-HOUSE
                    @elseif(trim(strtoupper($data_filter->status)) == 'CHECKOUT') CHECK-OUT
                    @elseif(trim(strtoupper($data_filter->status)) == 'CANCELED') CANCELED
                    @elseif(trim(strtoupper($data_filter->status)) == 'NOTSHOW') NOT SHOW
                    @elseif(trim(strtoupper($data_filter->status)) == 'NOTSHOWEND') NOT SHOW END
                    @elseif(trim(strtoupper($data_filter->status)) == 'NOTSHOWENDNEEDPROCESS') NOT SHOW END (NEED PROCESS)
                    @endif
                </span>
                @endif
            </div>
                ...
                <table class="table table-bordered align-middle table-nowrap mb-0">
                    <thead class="border">
                        <tr class="fs-7 fw-bolder text-gray-500">
                            ...
                            <th class="w-80px ps-3 pe-3 text-center">Platform</th>
                            ...
                            <th class="min-w-50px ps-3 pe-3 text-center">Keterangan</th>
                    ...
                    <tbody class="border">
                        ...
                        <tr class="fs-6 fw-bold text-gray-700">
                            ...
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <span class="badge badge-light-primary fs-7 fw-boldest">{{ trim($data->platform) }}</span>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                            ...
                            <td class="ps-3 pe-3" style="text-align:left;vertical-align:top;">
                                <p class="fs-7 fw-bolder text-gray-600">{{ trim($data->keterangan) }}</p>
                            </td>
                            <td class="ps-3 pe-3" style="text-align:center;vertical-align:top;">
                                <a href="{{ route('transaksi.reservasi.history.form-reservasi-history', trim($data->kode_reservasi)) }}" id="btnEdit" class="btn btn-icon btn-light-primary btn-sm mt-1" data-toggle="modal" data-kode={{ trim($data->kode_reservasi) }}>
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <form id="formEntryFilter" name="formEntryFilter" autofill="off" autocomplete="off" method="get" action="{{ route('transaksi.reservasi.inhouse.daftar-reservasi-inhouse') }}">
                ...
                <div class="modal-body">
                    <div class="fv-row">
                        ...
                        <select id="selectFilterTanggal" name="filter_tanggal" class="form-select">
                            <option value="TANGGALRESERVASI" @if(trim(strtoupper($data_filter->filter_date)) == 'TANGGALRESERVASI') selected @endif>Tanggal Reservasi</option>
                            <option value="TANGGALCHECKIN" @if(trim(strtoupper($data_filter->filter_date)) == 'TANGGALCHECKIN') selected @endif>Tanggal Check In</option>
                            <option value="TANGGALCHECKOUT" @if(trim(strtoupper($data_filter->filter_date)) == 'TANGGALCHECKOUT') selected @endif>Tanggal Check Out</option>
                        </select>
                        <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 mb-3 mt-4">
                            <div class="fv-row">
                                <label class="form-label">Tanggal Awal:</label>
                                <input id="inputStartDate" name="start_date" type="text" placeholder="Pilih Tanggal Check In" class="form-control"
                                    value="{{ $data_filter->start_date }}">
                            </div>
                            <div class="fv-row mt-8">
                                <label class="form-label">Tanggal Awal:</label>
                                <input id="inputEndDate" name="end_date" type="text" placeholder="Pilih Tanggal Check In" class="form-control"
                    ...
                    <div class="fv-row mt-8">
                        ...
                        <select id="selectFilter" name="filter" class="form-select">
                            <option value="KODERESERVASI" @if(trim(strtoupper($data_filter->sortby)) == 'KODERESERVASI') selected @endif>Kode Reservasi</option>
                            <option value="TANGGALRESERVASI" @if(trim(strtoupper($data_filter->sortby)) == 'TANGGALRESERVASI') selected @endif>Tanggal Entry</option>
                            <option value="TANGGALCHECKIN" @if(trim(strtoupper($data_filter->sortby)) == 'TANGGALCHECKIN') selected @endif>Tanggal Check In</option>
                            <option value="TANGGALCHECKOUT" @if(trim(strtoupper($data_filter->sortby)) == 'TANGGALCHECKOUT') selected @endif>Tanggal Check Out</option>
                        </select>
                    </div>
                    <div class="fv-row mt-8">
                        <div class="nav-group nav-group-fluid">
                            <label>
                                <input id="btnAsc" name="ascdesc" type="radio" class="btn-check" value="asc" @if(trim($data_filter->ascdesc) == 'asc') checked @endif>
                                <span class="btn btn-sm btn-color-muted btn-active btn-active-primary fw-bolder px-4">ASCENDING</span>
                            </label>
                            <label>
                                <input id="btnDesc" name="ascdesc" type="radio" class="btn-check" value="desc" @if(trim($data_filter->ascdesc) == 'desc') checked @endif>
                                <span class="btn btn-sm btn-color-muted btn-active btn-active-primary fw-bolder px-4">DESCENDING</span>
                            </label>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    const url = {
        'data_reservasi': "{{ route('transaksi.reservasi.history.daftar-reservasi-history') }}"
    }
</script>
<script src="{{ asset('assets/js/app/custom/autonumeric.js') }}?v={{ time() }}"></script>
<script src="{{ asset('assets/js/app/transaksi/reservasi/history/reservasi-daftar.js') }}?v={{ time() }}"></script>
@endpush
@endsection
