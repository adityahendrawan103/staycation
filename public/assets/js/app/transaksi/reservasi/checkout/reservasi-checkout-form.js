$(document).ready(function () {
    ...
    // Daftar Reservasi
    // ===============================================================
    function loadTransaksiReservasi(page = 1, per_page = 10, filter = '', search = '') {
        loading.block();
        window.location.href = window.location.origin + window.location.pathname + '?filter='+filter+
    ...
    $('#inputSearch').on('change keydown', function (e) {
        if (e.keyCode == 13 || e.type == 'change') {
            ...
            loading.block();
            loadTransaksiReservasi(1, per_page, filter, search);
        }
    });
    ...
    $('#selectPerPage').change(function() {
        ...
        loading.block();
        loadTransaksiReservasi(page, per_page, filter, search);
    });
